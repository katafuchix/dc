...
static Image *ReadSCTImage(const ImageInfo *image_info,ExceptionInfo *exception)
{
  ...
  */
  for (y=0; y < (long) image->rows; y++)
  {
    ...
    {
      q->red=(Quantum) ScaleCharToQuantum((unsigned char) ReadBlobByte(image));
      q++;
    }
    ...
    for (x=0; x < (long) image->columns; x++)
    {
      q->green=(Quantum) ScaleCharToQuantum((unsigned char)
        ReadBlobByte(image));
      q++;
    }
    if (SyncImagePixels(image) == MagickFalse)
    ...
    for (x=0; x < (long) image->columns; x++)
    {
      q->blue=(Quantum) ScaleCharToQuantum((unsigned char) ReadBlobByte(image));
      q++;
    }
    if (SyncImagePixels(image) == MagickFalse)
      break;
